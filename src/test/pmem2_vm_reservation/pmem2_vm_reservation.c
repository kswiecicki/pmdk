		...
		char *argv[])
{
	...
	for (int i = (int)n_maps - 1; i > 0; i--) {
		...
		ret = pmem2_vm_reservation_map_find_previous(rsv, map[i],
				&fmap);
		UT_PMEM2_EXPECT_RETURN(ret, 0);
 ...
 * for the previous and next possible mappings
 */
static int
		...
		const struct test_case *tc, int argc, char *argv[])
{
	...
	ret = pmem2_vm_reservation_map_find_previous(rsv, map, &fmap);
		...
		char *argv[])
{
	...
	/* the first and the last mapping is the same mapping */
