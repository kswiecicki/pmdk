	...
	struct pmem2_map **map_ptr)
{
	...
	*map_ptr = NULL;
	...
	if (cfg->reserv) {
		...
		if (rsv_offset % Pagesize) {
			ret = PMEM2_E_OFFSET_UNALIGNED;
			ERR(
				"virtual memory offset %zu is not a multiple of %llu",
					rsv_offset, Pagesize);
			goto err;
		}
		...
		if ((size_t)reserv % alignment) {
			...
			ERR(
				"mapping address %p (virtual memory reservation address + offset)" \
				" is not a multiple of %lu",
					reserv, alignment);
			goto err;
		}
