...
static int
reservation_mend(struct pmem2_vm_reservation *rsv, struct pmem2_map *map)
{
	void *addr = map->addr;
	size_t size = map->reserved_length;

	if ((char *)addr < (char *)rsv->addr ||
	...
	struct pmem2_map **map_ptr)
{
	...
	*map_ptr = NULL;
	...
	if (cfg->reserv) {
		...
		if (cfg->reserv_offset + file_len >
				pmem2_vm_reservation_get_size(cfg->reserv))
			return PMEM2_E_LENGTH_OUT_OF_RANGE;
		/* check if region in reservation isn't already mapped */
		if (vm_reservation_map_find(cfg->reserv,
				cfg->reserv_offset, content_length))
			return PMEM2_E_MAPPING_EXISTS;

		reserv = (char *)pmem2_vm_reservation_get_address(cfg->reserv) +
				cfg->reserv_offset;
...
err:
	 ...
	 */
	if (cfg->reserv)
		mmap(addr, reserved_length, PROT_NONE, MAP_PRIVATE |
...
pmem2_unmap(struct pmem2_map **map_ptr)
{
	...
	if (map->reserv) {
		...
		ret = reservation_mend(map->reserv, map);
		if (ret)
			return ret;
	} else {
