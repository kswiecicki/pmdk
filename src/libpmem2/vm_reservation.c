...
/*
 * pmem2_vm_reservation_map_find_previous -- find the closest mapping preceding
 *                                           the provided one
 */
int
pmem2_vm_reservation_map_find_previous(struct pmem2_vm_reservation *rsv,
		struct pmem2_map *map, struct pmem2_map **prev_map)
{
	...
	*prev_map = NULL;
	...
	struct ravl_interval_node *node;
	node = ravl_interval_find_previous(rsv->itree, map);
	if (!node) {
...
{
	PMEM2_ERR_CLR();
	LOG(3, "reservation %p pmem2_map %p", rsv, map);
