		...
		const struct pmem2_source *src)
{
	...
	*map_ptr = NULL;
	...
	if (rsv) {
		...
		/* TODO - Investigate the granularity differences */
		/* if (effective_offset % (1 << 16)) { */
...
;
		...
		addr_hint = (char *)rsv_addr + rsv_offset;
		/*
		...
		if (base == NULL) {
			...
			if (ret_windows == ERROR_INVALID_ADDRESS)
				...
				ret = pmem2_lasterror_to_err();
			goto err_merge_vm_rsv_regions;
		}

		ASSERTeq(base, addr_hint);
	} else {
...
err_merge_vm_rsv_regions:
	if (rsv)
		vm_reservation_merge(rsv, addr_hint, length);
err_unlock:
	if (rsv)
		util_rwlock_unlock(&vm_rsv_lock);
...
pmem2_map_delete(struct pmem2_map **map_ptr)
{
	...
	if (rsv) {
		...
		ret = vm_reservation_unmap(rsv, map->addr,
				map->reserved_length);
		if (ret)
			goto err_register_map_in_vm_rsv;
