...
#define PMEMSET_PART_VALID_STATES (uint64_t)(PMEMSET_PART_STATE_INDETERMINATE |\
		PMEMSET_PART_STATE_OK | PMEMSET_PART_STATE_OK_BUT_INTERRUPTED |\
...
pmemset_sds_new(struct pmemset_sds **sds_ptr, struct pmemset_source *src)
{
	...
	*sds_ptr = NULL;
	...
	sds->acceptable_states = PMEMSET_PART_STATE_OK;
	sds->refcount = 0;
...
pmemset_sds_state_check_and_possible_refresh(struct pmemset_source *src,
		enum pmemset_part_state *state_ptr, bool *refreshed)
{
	if (refreshed)
		*refreshed = false;
...
/*
 * pmemset_sds_set_acceptable_states -- sets acceptable part states that should
 *                                      not return an error
 */
int
pmemset_sds_set_acceptable_states(struct pmemset_sds *sds, uint64_t states)
{
	if (states & ~PMEMSET_PART_VALID_STATES)
		return PMEMSET_E_INVALID_PART_STATES;

	sds->acceptable_states = states;
