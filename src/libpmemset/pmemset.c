...
/*
 * pmemset_delete_all_pmaps_ravl_cb_arg -- arguments for ravl callback
 */
struct pmemset_delete_all_pmaps_ravl_cb_arg {
	int *ret;
 ...
 */
static void
pmemset_delete_all_pmaps_ravl_cb(void *data, void *arg)
{
	struct pmemset_part_map **pmap_ptr = (struct pmemset_part_map **)data;
	struct pmemset_part_map *pmap = *pmap_ptr;

	struct pmemset_delete_all_pmaps_ravl_cb_arg *cb_args =
			(struct pmemset_delete_all_pmaps_ravl_cb_arg *)arg;
...
pmemset_delete(struct pmemset **set)
{
	...
	struct pmemset_delete_all_pmaps_ravl_cb_arg arg = { .ret = &ret,
			.adjust_reservation = (rsv == NULL) };

	/* delete RAVL tree with part_map nodes */
	ravl_interval_delete_cb((*set)->shared_state.part_map_tree,
			pmemset_delete_all_pmaps_ravl_cb, &arg);
	if (ret)
		return ret;
		...
		size_t offset, size_t size, enum reservation_prep_type type)
{
	...
	if (type == RESERV_PREP_TYPE_EXTEND_IF_NEEDED &&
		...
		return pmem2_vm_reservation_extend(p2rsv, extend_size);
	}
		...
		struct pmemset_part_map *pmap)
{
	...
	node = ravl_interval_find_previous(set->shared_state.part_map_tree,
			pmap);
	if (!node)
