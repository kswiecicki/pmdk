		...
		struct pmem2_vm_reservation **pmem2_reserv)
{
	...
	if (!p2map) {
		...
		/* find latest pmem2 mapping in the vm reservation */
		...
		size_t offset, size_t size)
{
	...
	ASSERT(offset <= p2rsv_addr + p2rsv_size);

	if (p2rsv_addr + p2rsv_size > offset) {
		/* check if memory range after pmap isn't occupied */
		struct pmem2_map *p2map;
		pmem2_vm_reservation_map_find(p2rsv, offset, size,
				&p2map);
		if (p2map)
			ret = PMEM2_E_MAPPING_EXISTS;
	} else {
		/* extend the reservation to cover memory range after pmap */
		ret = pmem2_vm_reservation_extend(p2rsv, size);
	}
