		...
		os_semaphore_t *semaphore)
{
	 ...
	 *         threads that start from their primary indexe could be
	 *         snatching the lanes closest to the aforementioned thread,
	 *         resulting in that thread hanging while looking for the lane
	 *         for undefined time.
	 *         And there's no explicit error message to detect the bug.
	 */
	util_semaphore_wait(semaphore);
	for (info->lane_idx = 0; info->lane_idx < nlanes; ++info->lane_idx)
		if (util_bool_compare_and_swap64(
				&locks[info->lane_idx], 0, 1))
			return;

	FATAL("lane wasn't acquired despite passing semaphore wait");
}
